...
{
	int min_half_key = max<int>(GetMinSizing(NWST_BUTTON), ScaleGUIPixels(HALF_KEY_WIDTH));
	int key_width = min_half_key + (ScaleGUIPixels(INTER_KEY_SPACE) + min_half_key) * (num_half - 1);

	if (widtype == NWID_SPACER) {
		if (!hor->IsEmpty()) key_width += ScaleGUIPixels(INTER_KEY_SPACE);
		NWidgetSpacer *spc = new NWidgetSpacer(key_width, height, false);
		hor->Add(spc);
	} else {
		if (!hor->IsEmpty()) {
			NWidgetSpacer *spc = new NWidgetSpacer(ScaleGUIPixels(INTER_KEY_SPACE), height, false);
			hor->Add(spc);
		}
		NWidgetLeaf *leaf = new NWidgetLeaf(widtype, COLOUR_GREY, widnum, widdata, STR_NULL);
		leaf->SetMinimalSize(key_width, height, false);
		hor->Add(leaf);
	}
...
static NWidgetBase *MakeTopKeys(int *biggest_index)
{
	...
	int key_height = GetMinSizing(NWST_KEYBOARD, FONT_HEIGHT_NORMAL + ScaleGUIPixels(2));
...
static NWidgetBase *MakeNumberKeys(int *biggest_index)
{
	...
	int key_height = GetMinSizing(NWST_KEYBOARD, FONT_HEIGHT_NORMAL + ScaleGUIPixels(6));
