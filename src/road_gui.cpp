		...
		if (pt.x != -1) {
			switch (select_proc) {
				...
				case DDSP_SINGLE_TILE:
					...
					CommandContainer cmdcont = { start_tile, _cur_roadtype << 2 | _road_depot_orientation |
					(last_started_action == WID_ROT_BIG_DEPOT ? 1 : 6) << 6 |
					(last_started_action == WID_ROT_BIG_DEPOT ? INVALID_DEPOT : NEW_DEPOT) << 16,
					end_tile, CMD_BUILD_ROAD_DEPOT | CMD_MSG(_road_type_infos[_cur_roadtype].err_depot), CcRoadDepot, "" };
