...
struct DepotWindow : Window {
	...
	DepotWindow(WindowDesc *desc, DepotID depot_id) : Window(desc)
	{
		...
		assert(IsCompanyBuildableVehicleType(depot->veh_type)); // ensure that we make the call with a valid typex
		...
		for (y = r.top + 1; num < maxval; y += this->resize.step_height) { // Draw the rows
			for (uint i = 0; i < this->num_columns && num < maxval; i++, num++) {
				...
				if (this->type == VEH_TRAIN && this->problematic_vehicles[num] == 1) {
					GfxFillRect(r.left, y, r.right, y + this->resize.step_height - 1, PC_DARK_GREY);
				}
 ...
 */
static bool DepotJoinerNeeded(TileArea ta, const DepotPickerCmdProc proc, VehicleType veh_type)
{
	...
	return depot == nullptr && (_settings_game.depot.adjacent_depots || _depots_nearby_list.empty());
}
...
struct SelectDepotWindow : Window {
	DepotPickerCmdProc select_depot_proc;
	...
	void DrawWidget(const Rect &r, int widget) const override
	{
		...
		for (uint i = std::max<uint>(1, this->vscroll->GetPosition()); i <= _depots_nearby_list.size(); ++i, y += this->resize.step_height) {
			...
			[[maybe_unused]] Depot *depot = Depot::GetIfValid(_depots_nearby_list[i - 1]);
	...
	void OnClick(Point pt, int widget, int click_count) override
	{
		...
		uint dep_index = this->vscroll->GetScrolledRowFromWidget(pt.y, this, WID_JD_PANEL, WD_FRAMERECT_TOP);
		bool distant_join = (dep_index > 0);
		if (distant_join) dep_index--;

		if (distant_join && dep_index >= _depots_nearby_list.size()) return;

		/* Execute stored Command */
		this->select_depot_proc(distant_join ? _depots_nearby_list[dep_index] : NEW_DEPOT);
	...
	void OnResize() override
	{
		this->vscroll->SetCapacityFromWidget(this, WID_JD_PANEL, WD_FRAMERECT_TOP + WD_FRAMERECT_BOTTOM);
	}
	...
	void OnMouseOver(Point pt, int widget) override
	{
		...
		uint dep_index = this->vscroll->GetScrolledRowFromWidget(pt.y, this, WID_JD_PANEL, WD_FRAMERECT_TOP);
		if (dep_index == 0 || dep_index > _depots_nearby_list.size()) {
			SetViewportHighlightDepot(INVALID_DEPOT, true);
		} else {
			dep_index--;
			SetViewportHighlightDepot(Depot::Get(_depots_nearby_list[dep_index])->index, true);
		}
	}
...
{
	if (DepotJoinerNeeded(ta, proc, veh_type)) {
