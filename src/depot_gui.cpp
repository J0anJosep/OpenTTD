...
static bool DepotJoinerNeeded(CommandContainer cmd, VehicleType veh_type, TileArea ta)
{
	...
	if (!_settings_game.depot.distant_join_depots) return false;
	...
	return depot == nullptr && (_settings_game.depot.adjacent_depots || _depots_nearby_list.size() == 0);
}
...
struct SelectDepotWindow : Window {
		...
		area(ta)
	{
		...
		this->GetWidget<NWidgetCore>(WID_JD_CAPTION)->widget_data = STR_JOIN_DEPOT_CAPTION;
	...
	void OnClick(Point pt, int widget, int click_count) override
	{
		...
		uint st_index = this->vscroll->GetScrolledRowFromWidget(pt.y, this, WID_JD_PANEL, WD_FRAMERECT_TOP);
...
static const NWidgetPart _nested_select_depot_widgets[] = {
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_CAPTION, COLOUR_DARK_GREEN, WID_JD_CAPTION), SetDataTip(STR_JOIN_STATION_CAPTION, STR_TOOLTIP_WINDOW_TITLE_DRAG_THIS),
