...
struct CargoesField {
	...
	int GetCargoBase(int xpos) const
	{
		...
		switch (this->u.cargo.num_cargoes) {
			case 0: return xpos + ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH) / 2;
			case 1: return xpos + ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH - HOR_CARGO_WIDTH) / 2;
			case 2: return xpos + ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH / 2 - HOR_CARGO_WIDTH - HOR_CARGO_SPACE / 2);
			case 3: return xpos + ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH / 2 - HOR_CARGO_WIDTH - HOR_CARGO_SPACE - HOR_CARGO_WIDTH / 2);
			default: NOT_REACHED();
		}
	}
			...
			case CFT_CARGO: {
				...
				for (uint i = 0; i < MAX_CARGOES; i++) {
					...
					if (hor_right[i] != INVALID_CARGO) {
						...
						DrawHorConnection(cargo_base + col * ScaleGUIPixels(HOR_CARGO_SPACE) + (col + 1) * ScaleGUIPixels(HOR_CARGO_WIDTH) + dx, xpos + ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH) - 1, ypos, csp);
					}
...
struct IndustryCargoesWindow : public Window {
	...
	virtual void OnInit()
	{
		...
		uint min_ind_height = ScaleGUIPixels(CargoesField::VERT_CARGO_EDGE) * 2 + MAX_CARGOES * FONT_HEIGHT_NORMAL + (MAX_CARGOES - 1) * ScaleGUIPixels(CargoesField::VERT_CARGO_SPACE);
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		switch (widget) {
			case WID_IC_PANEL:
				size->width = CargoesField::industry_width * 3 + ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH * 2 + WD_FRAMETEXT_LEFT + WD_FRAMETEXT_RIGHT);
				break;
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		...
		if (this->ind_cargo >= NUM_INDUSTRYTYPES) left_pos += (CargoesField::industry_width + ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH)) / 2;
		...
		for (uint i = 0; i < this->fields.Length(); i++) {
			...
			if (vpos + row_height >= 0) {
				...
				while (col >= 0 && col <= last_column) {
					...
					xpos += (col & 1) ? ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH) : CargoesField::industry_width;
					col += dir;
				}
			}
			vpos += row_height;
	...
	bool CalculatePositionInWidget(Point pt, Point *fieldxy, Point *xy)
	{
		...
		int xpos = 2 * SWD_FRAMERECT_LEFT + ((this->ind_cargo < NUM_INDUSTRYTYPES) ? 0 :  (CargoesField::industry_width + ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH)) / 2);
		if (pt.x < xpos) return false;
		int column;
		for (column = 0; column <= 5; column++) {
			int width = (column & 1) ? ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH) : CargoesField::industry_width;
		...
		if (_current_text_dir == TD_RTL) {
			...
			xy->x = ((column & 1) ? ScaleGUIPixels(CargoesField::CARGO_FIELD_WIDTH) : CargoesField::industry_width) - xpos;
		} else {
			fieldxy->x = column;
			xy->x = xpos;
		}
