...
CargoArray GetAcceptanceAroundTiles(const TileArea ta, const BitMap *mask, uint32 *always_accepted)
{
	...
	MASKED_TILE_AREA_LOOP(tile, ta, mask) AddAcceptedCargo(tile, acceptance, always_accepted);
...
void UpdateStationAcceptance(Station *st, bool show_msg)
{
	...
	CargoArray acceptance;
	if (!st->rect.IsEmpty()) {
		acceptance = GetAcceptanceAroundTiles(st->GetStationCatchmentArea(), st->GetStationCatchmentFootprint(), &st->always_accepted);
	}
 ...
 * @param ta is the tiles of the station that have may changed
 * @param type is the type of StationType of tiles that may have changed
 */
void Station::AfterStationTileSetChange(bool adding, const TileArea ta, StationType type)
{
	...
	switch (type) {
		case STATION_RAIL:
			widget_index = WID_SV_TRAINS;
			break;
		case STATION_AIRPORT:
			widget_index = WID_SV_PLANES;
			break;
		case STATION_TRUCK:
		case STATION_BUS:
			widget_index = WID_SV_ROADVEHS;
			break;
		case STATION_DOCK:
			widget_index = WID_SV_SHIPS;
			break;
		default: NOT_REACHED();
	}

	SetWindowWidgetDirty(WC_STATION_VIEW, this->index, widget_index);
...
uint8 CalculateNoiseLevel(const Station *st)
{
	...
	const AirTypeInfo *air_type_info = GetAirTypeInfo(st->airport.air_type);

	return air_type_info->base_noise_level +
			st->airport.runways.Length() * air_type_info->runway_noise_level +
			st->airport.terminals.Length() + st->airport.helipads.Length();
}
...
CommandCost CmdRotateDockTracks(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (flags & DC_EXEC) {
		if (HasWaterTrackReservation(tile)) LiftReservations(tile);
...
void DrawAirportTracks(const TileInfo *ti)
{
	...
	Track track;
	FOR_EACH_SET_TRACK(track, trackbits | runway_tracks) {
		PaletteID palette = PAL_NONE;
		TrackBits tracks = TrackToTrackBits(track);

		if ((runway_tracks & tracks) != 0) {
			palette = PALETTE_SEL_TILE_RED;
		} else if ((trackbits & tracks) != 0) {
			palette = (reserved & tracks) != 0 ? PALETTE_SEL_TILE_BLUE : PALETTE_CRASH;
		}

		int offset = abs(_AutorailTilehSprite[autorail_tileh][track]);
		if (palette != PAL_NONE) DrawGroundSpriteAt(SPR_AUTORAIL_BASE + offset,
				   palette, 0, 0, TILE_HEIGHT);
	}
}
...
static void DrawAirportFences(TileInfo *ti)
{
	...
	static const uint8 x_off_air_fence[4] = {1, 0, TILE_SIZE - 1, 0};
	static const uint8 y_off_air_fence[4] = {0, TILE_SIZE - 1, 0, 1};
	...
	for (DiagDirection dir = DIAGDIR_BEGIN; dir < DIAGDIR_END; dir++) {
		...
		ti->x + x_off_air_fence[dir], ti->y + y_off_air_fence[dir], 1, 16, 4, ti->z);
	}
}
...
draw_default_foundation:
	...
	if (IsBuoy(ti->tile)) {
		...
		if (_settings_client.gui.show_water_tracks) DrawWaterTrackReservation(ti->tile);
...
static TrackStatus GetTileTrackStatus_Station(TileIndex tile, TransportType mode, uint sub_mode, DiagDirection side)
{
	...
	switch (mode) {
		...
		case TRANSPORT_WATER:
			...
			if (IsBuoy(tile)) {
				trackbits = GetWaterTracksForBuoy(tile);
