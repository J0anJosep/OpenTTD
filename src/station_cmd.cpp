 ...
 */
CargoArray GetAcceptanceAroundTiles(const TileArea ta, const BitMap *mask, CargoTypes *always_accepted)
{
...
CommandCost CmdRotateDockTracks(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (flags & DC_EXEC) {
		if (HasWaterTrackReservation(tile)) LiftShipPathsReservations(tile);
