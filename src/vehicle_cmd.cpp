...
CommandCost CmdRefitVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	Train *train = v->type == VEH_TRAIN ? Train::From(front) : nullptr;
	...
	/* If it is a train on a big depot, try placing it. */
...
const uint16 DEFAULT_SERVICE_TIME = 1 << 7;
const uint16 TRAIN_SERVICE_TIME = 1 < 8;
...
bool Vehicle::ContinueServicing()
{
	...
	if ((this->wait_counter & 15) == 0) {
		uint8 percent;
		uint16 max = this->type == VEH_TRAIN ? TRAIN_SERVICE_TIME : DEFAULT_SERVICE_TIME;
		/* Return the percentage */
		if ((max - this->wait_counter) * 2 < max) {
			/* Less than 50%; round up, so that 0% means really empty. */
			percent = (uint8)CeilDiv((max - this->wait_counter) * 100, max);
		} else {
			/* More than 50%; round down, so that 100% means really full. */
			percent = (uint8)(((max - this->wait_counter) * 100) / max);
		}
