...
CommandCost CmdSellVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (v->type == VEH_TRAIN) {
		...
		if (flags & DC_EXEC) {
			if (front->type == VEH_ROAD) ChangeNumDepotVehicles(v->tile, -1);
...
CommandCost CmdStartStopVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	switch (v->type) {
		...
		case VEH_AIRCRAFT: {
			...
			if (!(v->vehstatus & VS_CRASHED) && a->IsAircraftFlying()) return_cmd_error(STR_ERROR_AIRCRAFT_IS_IN_FLIGHT);
			break;
		}
	...
	if (flags & DC_EXEC) {
		...
		if (v->type == VEH_SHIP) LiftShipPathReservation(Ship::From(v), INVALID_TILE, INVALID_TRACK, true);
		v->MarkDirty();
...
CommandCost CmdMassStartStopVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (vehicle_list_window) {
		...
		DepotID depot_id = GetDepotIndex(tile);
		BuildDepotVehicleList(vli.vtype, depot_id, &list, NULL);
	}
...
CommandCost CmdDepotSellAllVehicles(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	/* Get the list of vehicles in the depot */
	BuildDepotVehicleList(vehicle_type, depot_id, &list, &list);
