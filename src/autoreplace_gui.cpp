	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		switch (widget) {
			case WID_RV_INFO_TAB: {
				...
				DrawString(r.left + SWD_FRAMETEXT_LEFT, r.right - SWD_FRAMETEXT_RIGHT, CenterBounds(r.top, r.bottom), str, TC_BLACK, SA_HOR_CENTER);
				break;
			}
			...
			case WID_RV_RIGHT_MATRIX: {
				...
				DrawEngineList((VehicleType)this->window_number, r.left, r.right, r.top,
			...
			for (int side = 0; side < 2; side++) {
				if (this->sel_engine[side] != INVALID_ENGINE) {
					...
					int text_end = DrawVehiclePurchaseInfo(nwi->pos_x + SWD_FRAMETEXT_LEFT, nwi->pos_x + nwi->current_x - SWD_FRAMETEXT_RIGHT,
							nwi->pos_y + SWD_FRAMERECT_TOP, this->sel_engine[side]);
	...
	NWidget(NWID_HORIZONTAL, NC_EQUALSIZE),
		...
		NWidget(WWT_PANEL, COLOUR_GREY),
			NWidget(NWID_HORIZONTAL),
				...
				NWidget(WWT_TEXTBTN, COLOUR_GREY, WID_RV_SHOW_HIDDEN_ENGINES), SetSizingType(NWST_STEP), SetDataTip(STR_SHOW_HIDDEN_ENGINES_VEHICLE_TRAIN, STR_SHOW_HIDDEN_ENGINES_VEHICLE_TRAIN_TOOLTIP),
			EndContainer(),
		EndContainer(),
	EndContainer(),
	NWidget(NWID_HORIZONTAL, NC_EQUALSIZE),
		...
		NWidget(NWID_VERTICAL),
			NWidget(NWID_HORIZONTAL),
				NWidget(WWT_PUSHTXTBTN, COLOUR_GREY, WID_RV_SORT_ASCENDING_DESCENDING), SetDataTip(STR_BUTTON_SORT_BY, STR_TOOLTIP_SORT_ORDER), SetSizingType(NWST_STEP), SetFill(1, 1),
...
static const NWidgetPart _nested_replace_vehicle_widgets[] = {
	...
	NWidget(NWID_HORIZONTAL, NC_EQUALSIZE),
		...
		NWidget(NWID_HORIZONTAL),
			NWidget(WWT_PUSHTXTBTN, COLOUR_GREY, WID_RV_SORT_ASCENDING_DESCENDING), SetDataTip(STR_BUTTON_SORT_BY, STR_TOOLTIP_SORT_ORDER), SetSizingType(NWST_STEP),
			NWidget(WWT_DROPDOWN, COLOUR_GREY, WID_RV_SORT_DROPDOWN), SetSizingType(NWST_STEP), SetResize(1, 0), SetFill(1, 1), SetDataTip(STR_JUST_STRING, STR_TOOLTIP_SORT_CRITERIA),
		EndContainer(),
	EndContainer(),
	NWidget(NWID_HORIZONTAL, NC_EQUALSIZE),
		NWidget(WWT_MATRIX, COLOUR_GREY, WID_RV_LEFT_MATRIX), SetMinimalSize(216, 0), SetFill(1, 1), SetMatrixDataTip(1, 0, STR_REPLACE_HELP_LEFT_ARRAY), SetResize(1, 1), SetScrollbar(WID_RV_LEFT_SCROLLBAR),
