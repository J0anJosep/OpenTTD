	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		switch (widget) {
			...
			case WID_RV_INFO_TAB: {
				...
				DrawString(r.left + WD_FRAMETEXT_LEFT, r.right - WD_FRAMETEXT_RIGHT, Center(r.top, r.bottom - r.top), str, TC_BLACK, SA_HOR_CENTER);
				break;
			}
	...
	NWidget(NWID_HORIZONTAL, NC_EQUALSIZE),
		...
		NWidget(NWID_HORIZONTAL),
			...
			NWidget(NWID_VERTICAL),
				...
				NWidget(WWT_PANEL, COLOUR_GREY), SetSizingType(NWST_STEP), SetResize(1, 0), SetFill(1, 1), EndContainer(),
			EndContainer(),
		EndContainer(),
	EndContainer(),
	NWidget(NWID_HORIZONTAL, NC_EQUALSIZE),
