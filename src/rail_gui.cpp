			...
			switch (select_proc) {
				...
				case DDSP_BUILD_STATION:
					if (this->last_user_action == WID_RAT_BUILD_BIG_DEPOT) {
						...
						CommandContainer cmdcont = { start_tile, _cur_railtype | (_build_depot_direction << 4) | (1 << 6) | (INVALID_DEPOT << 16), end_tile, CMD_BUILD_TRAIN_DEPOT | CMD_MSG(STR_ERROR_CAN_T_BUILD_TRAIN_DEPOT), CcRailDepot, "" };
				...
				case DDSP_SINGLE_TILE:
					...
					/* Tile is always the land tile, so need to evaluate _thd.pos. */
					CommandContainer cmdcont = { end_tile, _cur_railtype | (_build_depot_direction << 4) | (NEW_DEPOT << 16), end_tile, CMD_BUILD_TRAIN_DEPOT | CMD_MSG(STR_ERROR_CAN_T_BUILD_TRAIN_DEPOT), CcRailDepot, "" };
