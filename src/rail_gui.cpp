				...
				case DDSP_REMOVE_DEPOT:
					if (_remove_button_clicked) {
						...
						CommandContainer cmdcont = { start_tile, (uint32)(_cur_railtype | (_build_depot_direction << 6) |
								(_ctrl_pressed << 8) | ((this->last_user_action == WID_RAT_BUILD_EXTENDED_DEPOT) << 9) | (INVALID_DEPOT << 16)),
...
static NWidgetBase *MakeNWidgetRailDepot(int *biggest_index)
{
	...
	if (HasBit(_settings_game.depot.rail_depot_types, 1)) {
		// big depot
		hor->Add(new NWidgetLeaf(WWT_IMGBTN, COLOUR_DARK_GREEN, WID_RAT_BUILD_EXTENDED_DEPOT, SPR_IMG_DEPOT_RAIL, STR_RAIL_TOOLBAR_TOOLTIP_BUILD_EXTENDED_TRAIN_DEPOT));
	}

	*biggest_index = WID_RAT_BUILD_EXTENDED_DEPOT;
	return hor;
}
...
static WindowDesc _build_extended_depot_desc(
	...
	WC_BUILD_DEPOT, WC_BUILD_TOOLBAR,
	WDF_CONSTRUCTION,
	_nested_build_extended_depot_widgets, lengthof(_nested_build_extended_depot_widgets)
);

static void ShowBuildTrainDepotPicker(Window *parent, bool big_depot)
{
	new BuildRailDepotWindow(big_depot ? &_build_extended_depot_desc : &_build_depot_desc, parent, big_depot);
}
