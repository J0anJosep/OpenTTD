...
CommandCost CmdBuildLock(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (flags & DC_EXEC) {
		...
		TILE_AREA_LOOP(t, ta) {
			if (WaterTrackMayExist(t) && !LiftReservedWaterPaths(t)) NOT_REACHED();
		}
...
CommandCost CmdModifyPreferredWaterTrackdir(TileIndex tile, DoCommandFlag flags, TileIndex end_tile, uint32 p2, const char *text)
{
	...
	for (;;) {
		...
		if (WaterTrackMayExist(tile)) {
			...
			TrackdirBits change_trackdirs = TrackdirToTrackdirBits(trackdir) & TrackStatusToTrackdirBits(GetTileTrackStatus(tile, TRANSPORT_WATER, 0));

			bool change = false;
			if (change_trackdirs != TRACKDIR_BIT_NONE) {
				if (!remove) {
...
static CommandCost ClearTile_Water(TileIndex tile, DoCommandFlag flags)
{
	...
	if (flags & DC_EXEC) {
		if (HasWaterTrackReservation(tile) && !LiftReservedWaterPaths(tile)) NOT_REACHED();
	}
