...
TrackBits GetWaterTracks(TileIndex t)
{
	...
	return (TrackBits)GB(_m[t].m2, 6, 6);
}
...
void UpdateWaterTracks(TileIndex t)
{
	...
	if (!_settings_game.pf.coastal_water_tracks) {
		/* Get the shores of this tile so we can remove tracks that cross those shores. */
...
/**
 * Return whether a lock part needs a gate in its lowering direction.
 * @param tile tile to check
 * @return true if the water level of this tile and the tile in the lowering
 *         direction is diferent.
 */
...
/**
 * Draw the water tracks of a tile if setting show tracks is enabled.
...
void DrawWaterTracks(TileIndex tile)
{
	...
	FOR_EACH_SET_TRACK(track, available) {
		...
		DrawGroundSpriteAt(SPR_AUTORAIL_BASE + autorail_offset[track] + slope_off, PALETTE_SEL_TILE_BLUE, 0, 0, TILE_HEIGHT + z_offset);
	}


	FOR_EACH_SET_TRACK(track, shallow) {
		if ((TrackToTrackBits(track) & trackbits) != 0) continue;
		DrawGroundSpriteAt(SPR_AUTORAIL_BASE + autorail_offset[track] + slope_off, PALETTE_SEL_TILE_RED, 0, 0, TILE_HEIGHT + z_offset);
	}

	FOR_EACH_SET_TRACK(track, trackbits) {
		DrawGroundSpriteAt(SPR_AUTORAIL_BASE + autorail_offset[track] + slope_off, PAL_NONE, 0, 0, TILE_HEIGHT + z_offset);
	}
}
