...
CommandCost CmdBuildShipDepot(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	bool build_extended_depot = HasBit(p1, 2);
	DepotID join_to = GB(p1, 16, 16);

	if (Company::IsValidHumanID(_current_company) && !HasBit(_settings_game.depot.water_depot_types, build_extended_depot)) {
	...
	for (TileIndex t : complete_area) {
		...
		if (flags & DC_EXEC) {
			...
			MakeShipDepot(t,  _current_company, depot->index, build_extended_depot, dp, axis, wc);
