...
CommandCost CmdBuildTrainDepot(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	bool build_extended_depot = HasBit(p1, 9);

	if (Company::IsValidHumanID(_current_company) && !HasBit(_settings_game.depot.rail_depot_types, build_extended_depot)) return_cmd_error(STR_ERROR_DEPOT_TYPE_NOT_AVAILABLE);
	...
	for (TileIndex tile : ta) {
		...
		if (tileh != SLOPE_FLAT) {
			...
			if (build_extended_depot && !CanBuildDepotByTileh(ReverseDiagDir(dir), tileh)) {
				return_cmd_error(STR_ERROR_FLAT_LAND_REQUIRED);
			}
			cost.AddCost(_price[PR_BUILD_FOUNDATION]);
		}

		if (build_extended_depot && IsPlainRailTile(tile) && !HasSignals(tile) && GetRailType(tile) == railtype) {
	...
	for (TileIndex tile : ta) {
		...
		if (flags & DC_EXEC) {
			...
			SB(_m[tile].m5, 5, 1, build_extended_depot);
			MarkTileDirtyByTile(tile);

			Company::Get(_current_company)->infrastructure.rail[railtype]++;

			if (build_extended_depot) {
...
CommandCost CmdConvertRail(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
			...
			case MP_RAILWAY:
				switch (GetRailTileType(tile)) {
					case RAIL_TILE_DEPOT:
						if (IsExtendedDepot(tile) && find(depots_affected.begin(), depots_affected.end(), (tile)) == depots_affected.end()) {
	...
	/* Update big depots. */
	for(auto &depot_tile : depots_affected) {
	...
	if (IsExtendedRailDepot(tile)) {
		...
		if (y == TILE_SIZE / 2) {
			...
			if (stop_pos == x) {
				...
				uint16 spd = std::max(0, stop_pos * 20 - 15);
