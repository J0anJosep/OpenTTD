 ...
 * @pre IsAirportTile
 */
static inline void SetAirtype(TileIndex t, AirType type)
{
 ...
 * @pre IsAirportTile
 */
static inline AirType GetAirtype(TileIndex t)
{
	assert(IsValidTile(t));
	assert(IsTileType(t, MP_STATION));
	assert(IsAirport(t));
...
static inline void SetAirportTileType(TileIndex t, AirportTileType type)
{
	...
	assert(IsAirport(t));
	assert(type < ATT_END);
	SB(_m[t].m5, 4, ATT_NUM_BITS, type);
}
...
static inline AirportTileType GetAirportTileType(TileIndex t)
{
	...
	assert(IsTileType(t, MP_STATION));
	assert(IsAirport(t));
	AirportTileType type = (AirportTileType)(GB(_m[t].m5, 4, ATT_NUM_BITS));
	assert(type < ATT_END);
	return type;
}
...
static inline bool IsInfrastructure(TileIndex t)
{
	...
	return GB(_m[t].m5, 8 - ATT_INFRA_LAYOUT_NUM_BITS, ATT_INFRA_LAYOUT_NUM_BITS) == ATT_INFRA_LAYOUT_BITS;
}
...
static inline bool IsRunway(TileIndex t)
{
	...
	return GB(_m[t].m5, 8 - ATT_RUNWAY_LAYOUT_NUM_BITS, ATT_RUNWAY_LAYOUT_NUM_BITS) == ATT_RUNWAY_LAYOUT_BITS;
}
...
static inline bool IsRunwayStart(TileIndex t)
{
	...
	return GB(_m[t].m5, 8 - ATT_RUNWAY_START_LAYOUT_NUM_BITS, ATT_RUNWAY_START_LAYOUT_NUM_BITS) == ATT_RUNWAY_START_LAYOUT_BITS;
}
...
static inline DiagDirection GetRunwayExtremeDirection(TileIndex t)
{
	...
	return (DiagDirection)GB(_me[t].m8, 12, 2);
}
...
static inline Direction GetPlainRunwayDirections(TileIndex t)
{
	...
	return (Direction)GB(_me[t].m8, 12, 3);
}
...
static inline void AddPlainRunwayDirections(TileIndex t, DiagDirection dir, bool first)
{
	...
	if (first) {
		SB(_me[t].m8, 12, 3, DiagDirToDir(dir));
	} else {
		...
		if (add_dir + 2 == pre_dir) {
			SB(_me[t].m8, 12, 3, add_dir + 1);
		} else if (pre_dir == DIR_NW && add_dir == DIR_NE) {
			SB(_me[t].m8, 12, 3, DIR_N);
		} else {
			NOT_REACHED();
		}
	}
...
static inline bool RemovePlainRunwayDirections(TileIndex t, DiagDirection dir)
{
	...
	if (remove_dir == cur_dir) {
		...
		SB(_me[t].m8, 12, 3, (cur_dir - 1) & (DIR_END - 1));
		return false;
	} else if (cur_dir == (remove_dir + 1) % DIR_END) {
		SB(_me[t].m8, 12, 3, (cur_dir + 1) % DIR_END);
		return false;
	}
 ...
 * @pre IsHangar
 */
static inline void SetExtendedHangar(TileIndex t, bool is_extended)
{
	assert(IsHangar(t));
	SB(_m[t].m5, 5, 1, is_extended);
}
 ...
 * @pre IsHangar
 */
static inline bool IsExtendedHangar(TileIndex t)
{
	assert(IsHangar(t));
	return GB(_m[t].m5, 5, 1);
}
...
static inline TrackBits GetAirportTileTracks(TileIndex t)
{
	...
	return (TrackBits)GB(_me[t].m8, 0, 6);
}
...
static inline TrackBits GetReservedAirportTracks(TileIndex t)
{
	...
	return (TrackBits)GB(_me[t].m8, 6, 6);
}
...
static inline AirportTiles GetAirportGfxFromTile(TileIndex t)
{
	...
	return (AirportTiles)GB(_me[t].m8, 0, 8);
}
 ...
 * @pre IsInfrastructure
 */
static inline void SetAirportGfxToTile(TileIndex t, AirportTiles at)
{
	assert(IsAirportTile(t));
	assert(!MayHaveAirTracks(t));
	assert(IsInfrastructure(t));
