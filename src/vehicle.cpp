...
void VehicleServiceInBigDepot(Vehicle *v)
{
	...
	if (v->current_order.IsType(OT_GOTO_DEPOT)) {
		...
		if ((t.GetDepotOrderType() & ODTFB_PART_OF_ORDERS) &&
				...
				(v->type == VEH_AIRCRAFT ? t.GetDestination() != GetStationIndex(v->tile) : t.GetDestination() != GetDepotIndex(v->tile))) {
...
CommandCost FixReplacementOnBigDepot(const Vehicle *old_head, Vehicle *new_head)
{
	...
	switch (old_head->type) {
		...
		case VEH_SHIP: {
			...
			new_ship->GetImage(new_ship->direction, EIT_ON_MAP, &new_ship->sprite_seq);
			break;
		}
...
void Vehicle::PreDestructor()
{
	...
	if (this->Previous() == NULL && IsDepotTile(this->tile)) {
	...
	switch (this->breakdown_ctr) {
		...
		case 2:
			...
			if (this->type == VEH_AIRCRAFT) {
				...
				if (this->type == VEH_SHIP) LiftShipPathReservation(Ship::From(this), INVALID_TILE, INVALID_TRACK, true);
...
void AgeVehicle(Vehicle *v)
{
	if (v->age < MAX_DAY) {
		v->age++;
		if (v->IsPrimaryVehicle() && v->age == VEHICLE_PROFIT_MIN_AGE + 1) GroupStatistics::VehicleReachedProfitAge(v, 1);
	}
 ...
 */
CargoTypes GetCargoType(const Vehicle *v, bool check_refit_orders)
{
	CargoTypes cargo_types = 0;
