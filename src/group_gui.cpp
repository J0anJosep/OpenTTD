...
private:
	...
	uint ComputeGroupInfoSize()
	{
		...
		if (8 * GetMinSizing(NWST_BUTTON) > total_size + GetMinSizing(NWST_STEP)) {
			this->column_size[VGC_NAME].width += 8 * GetMinSizing(NWST_BUTTON) - total_size - GetMinSizing(NWST_STEP);
			total_size = 8 * GetMinSizing(NWST_BUTTON) - GetMinSizing(NWST_STEP);
		}
	...
	void DrawGroupInfo(int y, int left, int right, GroupID g_id, int indent = 0, bool is_active = true, bool protection = false) const
	{
		...
		DrawString(x + ScaleGUIPixels(indent * LEVEL_WIDTH), x + this->column_size[VGC_NAME].width + longer_name - 1, Center(y, this->tiny_step_height, this->column_size[VGC_NAME].height), str, colour);
