...
private:
	...
	void DrawGroupInfo(int y, int left, int right, GroupID g_id, int indent = 0, bool is_active = true, bool protection = false) const
	{
		...
		DrawString(x + (rtl ? 0 : indent), x + this->column_size[VGC_NAME].width + longer_name - 1 - (rtl ? indent : 0), y + (this->tiny_step_height - this->column_size[VGC_NAME].height) / 2, str, colour);
