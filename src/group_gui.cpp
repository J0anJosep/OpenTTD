...
private:
	...
	void DrawGroupInfo(int y, int left, int right, GroupID g_id, int indent = 0, bool is_active = true, bool protection = false) const
	{
		...
		if (g_id == this->group_over) {
			GfxFillRect(left + SWD_FRAMERECT_LEFT, y + ScaleGUIPixels(WD_FRAMERECT_TOP + WD_MATRIX_TOP), right - SWD_FRAMERECT_RIGHT, y + this->tiny_step_height - ScaleGUIPixels(WD_FRAMERECT_BOTTOM + WD_MATRIX_TOP), _colour_gradient[COLOUR_GREY][7]);
		}
