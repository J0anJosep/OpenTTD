...
static const NWidgetPart _nested_group_widgets[] = {
	...
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(NWID_VERTICAL),
			NWidget(WWT_PANEL, COLOUR_GREY, WID_GL_EMPTY), SetFill(1, 0), EndContainer(),
			...
			NWidget(WWT_PANEL, COLOUR_GREY, WID_GL_EMPTY), SetFill(1, 0), EndContainer(),
			NWidget(NWID_HORIZONTAL),
				NWidget(WWT_PUSHTXTBTN, COLOUR_GREY, WID_GL_GROUP_SORT_BY_ORDER), SetSizingType(NWST_STEP), SetMinimalSize(80, 0), SetDataTip(STR_BUTTON_SORT_BY, STR_TOOLTIP_SORT_ORDER),
				NWidget(WWT_DROPDOWN, COLOUR_GREY, WID_GL_GROUP_SORT_BY_DROPDOWN), SetMinimalSize(130, 0), SetFill(1, 0), SetDataTip(0x0, STR_TOOLTIP_SORT_CRITERIA),
				NWidget(WWT_PANEL, COLOUR_GREY), SetSizingType(NWST_STEP), SetFill(0, 0), EndContainer(),
			EndContainer(),
			NWidget(NWID_HORIZONTAL),
			...
			NWidget(NWID_HORIZONTAL),
				...
				NWidget(WWT_IMGBTN, COLOUR_GREY, WID_GL_MANAGE_GROUPS_DROPDOWN), SetFill(0, 1),
		...
		NWidget(NWID_VERTICAL),
			NWidget(NWID_HORIZONTAL),
				NWidget(WWT_PUSHTXTBTN, COLOUR_GREY, WID_GL_VEHICLE_SORT_BY_ORDER), SetSizingType(NWST_STEP), SetMinimalSize(81, 12), SetDataTip(STR_BUTTON_SORT_BY, STR_TOOLTIP_SORT_ORDER),
...
private:
	...
	uint ComputeGroupInfoSize()
	{
		...
		if (9 * GetMinSizing(NWST_BUTTON) > total_size + GetMinSizing(NWST_STEP)) {
			this->column_size[VGC_NAME].width += 9 * GetMinSizing(NWST_BUTTON) - total_size - GetMinSizing(NWST_STEP);
			total_size = 9 * GetMinSizing(NWST_BUTTON) - GetMinSizing(NWST_STEP);
		}
	...
	void DrawGroupInfo(int y, int left, int right, GroupID g_id, int indent = 0, bool is_active = true, bool protection = false) const
	{
		...
		DrawString(x + (rtl ? 0 : indent), x + this->column_size[VGC_NAME].width + longer_name - 1 - (rtl ? indent : 0), Center(y, this->tiny_step_height, this->column_size[VGC_NAME].height), str, colour);
...
public:
		...
		switch (widget) {
			...
			case WID_GL_LIST_GROUP: {
				...
				for (int i = this->group_sb->GetPosition(); i < max; ++i) {
					...
					DrawGroupInfo(y1, r.left, r.right, g->index, this->groups[i].indent * LEVEL_WIDTH, this->groups[i].active, g->replace_protection);
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (widget) {
			...
			case WID_GL_CREATE_GROUP: { // Create a new group
				...
				if (DoCommandP(0, this->vli.vtype, this->vli.index, CMD_CREATE_GROUP | CMD_MSG(STR_ERROR_GROUP_CAN_T_CREATE), CcCreateGroup))
	...
	virtual void OnHover(Point pt, int widget)
	{
		...
		args[1] = stat->profit_last_year / (OverflowSafeInt64)stat->num_profit_vehicle;
