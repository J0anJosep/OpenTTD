...
const Vehicle *Group::GetVehicleOfGroup() const
{
	Vehicle *v;
	FOR_ALL_VEHICLES(v) {
		if (v->group_id == this->index && v == v->First()) return v;
	}
...
CommandCost CmdBuildGroupsOfVehicleType(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	FOR_ALL_VEHICLES(v) {
		...
		FOR_ALL_VEHICLES_FROM(v2, i) {
			...
			if (grouped_by_type == GBT_ORDER_SIMPLE ||
					grouped_by_type == GBT_CARGO_ORDER ||
					grouped_by_type == GBT_ORDER_STATIONS) {
	...
	if (grouped_by_type == GBT_CARGO_ORDER || grouped_by_type == GBT_1ST_ENGINE_CLASS_CARGO) {
		...
		while (involved_groups.Length() > 0) {
			if (!Group::CanAllocateItem()) return CMD_ERROR;
			Group *new_parent_group = new Group(_current_company);
			new_parent_group->replace_protection = false;
			new_parent_group->vehicle_type = vt;
			...
			for (uint i = involved_groups.Length(); i--; ) {
				...
				switch (grouped_by_type) {
					case GBT_CARGO_ORDER:
						...
						break;
					case GBT_1ST_ENGINE_CLASS_CARGO: {
						const Vehicle *v2 = Group::Get(involved_groups[i])->GetVehicleOfGroup();
						assert(v2 != NULL);
						if (GetEngineLiveryScheme(v->engine_type, INVALID_ENGINE, v) != GetEngineLiveryScheme(v2->engine_type, INVALID_ENGINE, v2)) continue;
						break;
					}
				...
				iter->parent = new_parent_group->index;
