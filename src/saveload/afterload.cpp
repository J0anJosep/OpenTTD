...
bool AfterLoadGame()
{
	...
	if (IsSavegameVersionBefore(SLV_MOD_GROUP_HIERARCHY)) {
		Company *c;
		FOR_ALL_COMPANIES(c) {
	...
	if (IsSavegameVersionBefore(SLV_USE_DEPOT_IDS)) {
	...
	if (IsSavegameVersionBefore(SLV_86)) {
		...
		for (TileIndex t = 0; t < map_size; t++) {
			...
			if (IsLockTile(t)) SetWaterClassDependingOnSurroundings(t, false);
	...
	if (IsSavegameVersionBefore(SLV_MULTIPLE_DOCKS)) {
		...
		FOR_ALL_STATIONS(st) {
			...
			} else if (IsSavegameVersionBefore(SLV_SET_DOCK_TRACKS)) {
	...
	if (IsSavegameVersionBefore(SLV_DOCK_LOADING_BAY)) {
	...
	if (IsSavegameVersionBefore(SLV_STUCK_SHIPS)) {
		Ship *v;
		FOR_ALL_SHIPS(v) {
			v->stuck = false;
		}
	...
	if (IsSavegameVersionBefore(SLV_LOCK_STATE)) {
		for (TileIndex t = 0; t < map_size; t++) {
	...
	if (IsSavegameVersionBefore(SLV_ADD_WATER_TRACKDIRS)) {
	...
	if (IsSavegameVersionBefore(SLV_MULTILENGTH_LOCKS)) {
