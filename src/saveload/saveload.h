...
enum SLRefType {
	...
	REF_DOCKS          = 12, ///< Load/save a reference to a dock.
};
...
enum SaveLoadVersions {
	...
	SL_CONVENIENCE_SAVEBUMP_27572,  // 27572 -> Fix:        Convenience savegame bump to distinguish 1.6 savegames from 1.5 savegames.
	SL_CONVENIENCE_SAVEBUMP_27778,  // 27778 -> Fix:        Convenience savegame bump for 1.7.x savegames (version 196).
	SL_STORE_VARIETY_SETTING,       // 27978 -> Fix:        Store the map variety setting in the samegame.
	SL_EXTEND_MAP_ARRAY,            // #6805 -> Change:     Extend railtypes to 64, adding uint16 to map array.
	SL_SWITCH_TOWN_COUNTER,         //       -> Change:     Switch town growth rate and counter to actual game ticks (#6763).
	SL_INCREASE_CARGO_TYPES_64,     //       -> Change:     Increase cargo type limit to 64.
	SL_EXTEND_NEWGRF_PS,            // #6885 -> Feature:    [NewGRF] Increase size of persistent storage to 256.
	SL_INCREASE_IND_CARGO_SLOTS,    // #6867 -> Add:        Industries can produce and accept up to 16 different cargoes
	...
	SL_ADD_WATER_TRACKDIRS,         //       -> Feature:    Players may define special water trackdirs on tiles.
	SL_STORE_OTHER_TUNNELBRIDGE,    //       -> Codechange: Store in the map array the other tile of a tunnel or bridge.
	SL_MULTILENGTH_LOCKS,           //       -> Feature:    Add locks with 5 and 7 tile-length.
