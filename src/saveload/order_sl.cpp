...
const SaveLoad *GetOrderBackupDescription()
{
	static const SaveLoad _order_backup_desc[] = {
		...
		SLEG_CONDVAR(_tile,                                 SLE_UINT32,         SL_MIN_VERSION, SLV_USE_DEPOT_IDS),
		 SLE_CONDVAR(OrderBackup, depot_id,                 SLE_UINT16,      SLV_USE_DEPOT_IDS, SL_MAX_VERSION),
...
void Load_BKOR()
{
	...
	while ((index = SlIterateArray()) != -1) {
		...
		if (IsSavegameVersionBefore(SLV_USE_DEPOT_IDS)) {
			assert(IsDepotTile(_tile));
			ob->depot_id = GetDepotIndex(_tile);
		}
	}
}
