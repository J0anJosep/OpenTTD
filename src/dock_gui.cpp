		...
		if (pt.x != -1) {
			switch (select_proc) {
				...
				case DDSP_CREATE_WATER:
					TouchCommandP(end_tile, start_tile, (_game_mode == GM_EDITOR && _ctrl_pressed) ? WATER_CLASS_SEA : WATER_CLASS_CANAL, CMD_BUILD_CANAL | CMD_MSG(STR_ERROR_CAN_T_BUILD_CANALS), CcPlaySound_SPLAT_WATER);
					break;
				case DDSP_CREATE_RIVER:
					TouchCommandP(end_tile, start_tile, WATER_CLASS_RIVER, CMD_BUILD_CANAL | CMD_MSG(STR_ERROR_CAN_T_PLACE_RIVERS), CcPlaySound_SPLAT_WATER);
					break;
...
public:
	...
	virtual void DrawWidget(const Rect &r, int widget) const {
		...
		int y = Center(r.top + WD_FRAMERECT_TOP - WD_MATRIX_BOTTOM + IsWidgetLowered(widget) + ScaleGUITrad(22),
				r.bottom - r.top, ScaleGUITrad(3 * TILE_PIXELS / 2 + 22));
