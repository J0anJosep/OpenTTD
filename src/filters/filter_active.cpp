...
/**
 * Given an industry, say whether it satisfies a property
 * @param ind the industry
 * @param property like: closing, new, serviced...
 * @return true if the town satisfies the property
 */
bool TestProperty(const Industry *ind, IndustryProperties property)
{
	switch (property) {
		case IP_CLOSING:
			return ind->prod_level == PRODLEVEL_CLOSURE;

		case IP_NEW: {
			YearMonthDay ymd_now, ymd_industry;
			ConvertDateToYMD(_date, &ymd_now);
			ConvertDateToYMD(ind->construction_date, &ymd_industry);
			return ymd_now.year - ymd_industry.year < 1;
		}
...
bool FilterActive::FilterTest(const Vehicle *v) const
{
	...
	CargoTypes cargo_types = GetCargoType(v, true);
...
bool FilterActive::FilterTest(const Group *g) const
{
	...
	SmallVector<StationID, 32> visited_stations;
	SmallVector<TownID, 32> visited_towns;
	for (uint i = stat->order_lists.Length(); i--;) {
		const Order *order;
		FOR_ORDERLIST_ORDERS(stat->order_lists[i], order) {
			if ( (order->IsType(OT_GOTO_STATION) || order->IsType(OT_IMPLICIT)) && !order->IsType(OT_GOTO_WAYPOINT) ) {
				visited_stations.Include(order->GetDestination());
				const Station *st = Station::GetIfValid(order->GetDestination());
				if (st != NULL) visited_towns.Include(st->town->index);
		...
		for (uint j = lists[i].Length(); j--;) {
			...
			switch (i) {
				case LT_INDUSTRYTYPE:
					if (industry->type == lists[i][j].GetElement()) {
						return true;
					} else {
						if (j == 0) return false;
					}
				...
				case LT_INDUSTRY_PROPERTIES:
					if (state_is != TestProperty(industry, (IndustryProperties)lists[i][j].GetElement())) return false;
					break;
				default: NOT_REACHED();
			}
		}
