...
struct FilterLists : FilterBase {
			...
			switch (type) {
				case LT_INDUSTRYTYPE:
					for (uint i = 0; i < NUM_INDUSTRYTYPES; i++) {
						...
						*list.Append() = FilterElement(_sorted_industry_types[i]);
					}
					break;

				case LT_TOWNS:
					for (uint i = (uint)Town::GetPoolSize(); i--;) *list.Append() = FilterElement(i);
					break;
				...
				case LT_INDUSTRY_PROPERTIES:
					for (uint prop = IP_BEGIN; prop < IP_END; prop++) *list.Append() = FilterElement(prop);
					break;
	...
	*/
	...
	{
		bool prev_state = element.IsActive();

		if (type == LT_INDUSTRYTYPE || type == LT_CATCHMENT_AREA_PROPERTIES) {
