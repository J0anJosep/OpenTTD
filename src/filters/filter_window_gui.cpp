...
struct FilterWindowBase : Window, FilterLists {
	...
	void DrawList(const ListType list_type, const Rect &r, int &y, uint &n, uint initial_list_pos, bool only_active, uint &skip_lines) const
	{
		...
		for (uint i = initial_list_pos; i < list.Length() && n < this->vscroll->GetCapacity(); i++) {
			...
			StringID str;
			StringID mark = element_is.GetMark();
			TextColour colour_mark = element_is.GetColourString();
			int colour = element_is.GetColour();

			switch (list_type) {
				case LT_INDUSTRYTYPE: {
					const IndustrySpec *indsp = GetIndustrySpec(element_is.GetElement());
					if (!indsp->enabled) NOT_REACHED();
					DrawSquare(left_rect, y + sqr_offset, sqr_width, colour, indsp->map_colour);
					SetDParam(0, indsp->name);
					str = STR_FILTER_INDUSTRYTYPE;
					break;
				}
				...
				case LT_CARGO: {
					...
					DrawSquare(left_rect, y + sqr_offset, sqr_width, colour, cs->rating_colour);
				...
				case LT_COMPANIES: {
					...
					DrawFrameRect(left_rect, y + sqr_offset, right_rect, y + sqr_width + sqr_offset - 1, (Colours)c->colour, element_is.GetFrameFlags());
				...
				case LT_STATIONS: {
					...
					if (st->owner < MAX_COMPANIES) DrawFrameRect(left_rect, y + sqr_offset, right_rect, y + sqr_width + sqr_offset - 1, (Colours)Company::GetIfValid(st->owner)->colour, element_is.GetFrameFlags());
				...
				case LT_INDUSTRY_PROPERTIES:
					str = STR_FILTER_PROPERTIES_INDUSTRY_CLOSING + element_is.GetElement();
					break;
	...
	virtual void OnPaint()
	{
		...
		this->SetWidgetsDisabledState(false,
				FW_WIDGET_INDUSTRYTYPE,
				FW_WIDGET_STATIONS,
				FW_WIDGET_ORDERS,
				FW_WIDGET_VEHICLE_GROUP_PROPERTIES,
				FW_WIDGET_CATCHMENT_AREA_PROPERTIES,
				FW_WIDGET_TOWN_PROPERTIES,
				FW_WIDGET_INDUSTRY_PROPERTIES,
				FW_WIDGET_STATE,
				WIDGET_LIST_END);
		DisableWidget(FW_WIDGET_INDUSTRYTYPE + this->tab);
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (widget) {
			...
			case FW_WIDGET_INDUSTRY_PROPERTIES:
				this->tab = (FilterWindowTabs)(widget - FW_WIDGET_INDUSTRYTYPE);
				this->SetDirty();
				break;
