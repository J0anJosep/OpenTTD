...
struct GroupDetailsWindow : Window {
	...
	static SortersForOrderLists * schedules_sorter_funcs[];
		...
		switch (tab) {
			case GI_TAB_GENERAL:
				if (stat->num_profit_vehicle > 0) {
					...
					Money mean_prof = stat->profit_last_year / (OverflowSafeInt64)stat->num_profit_vehicle;
					SetDParam(1, mean_prof);
...
SortersForOrderLists * GroupDetailsWindow::schedules_sorter_funcs[] = {
	&OrdersTypeSorter,
