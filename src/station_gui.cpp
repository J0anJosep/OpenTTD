...
protected:
	...
	void SetCargoFilter()
	{
		...
		FOR_ALL_CARGOSPECS(cs) {
			if (active == NULL || active->FilterTest(cs)) this->cargo_filter |= (CargoTypes)(1ULL << cs->Index());
		}
	}
...
public:
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		switch (widget) {
			...
			case WID_STL_LIST:
				...
				size->height = ScaleGUIPixels(WD_FRAMERECT_TOP + WD_FRAMERECT_BOTTOM) + 5 * resize->height;
				break;
		}
	}
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (widget) {
			...
			case WID_STL_LIST: {
				uint16 id_v = this->vscroll->GetScrolledRowFromWidget(pt.y, this, WID_STL_LIST, SWD_FRAMERECT_TOP, this->resize.step_height);
...
static void DrawCargoIcons(CargoID i, uint waiting, int left, int right, int top, int bottom)
{
	...
	int width = d.width;
	top = Center(top, bottom - top + 1, d.height);
