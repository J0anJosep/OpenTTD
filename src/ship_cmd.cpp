	...
	if (v->state != TRACK_BIT_WORMHOLE) {
		...
		if (gp.old_tile == gp.new_tile) {
			...
			if (v->current_order.IsType(OT_GOTO_WAYPOINT)) {
				...
				if (HasWaterTrackReservation(gp.new_tile)) LiftShipPathReservation(gp.new_tile, v->GetVehicleTrackdir());
			...
			if (tracks & GetReservedWaterTracks(gp.new_tile)) {
				...
				if (_settings_game.pf.ship_path_reservation) LiftShipPathsReservations(gp.new_tile, true);
				...
				if (_settings_game.pf.ship_path_reservation) {
					if (!IsLockTile(gp.old_tile)) {
						...
						* We must lift reservation of all the lock tiles. */
						LiftShipPathReservation(gp.old_tile, ReverseTrackdir(v->GetVehicleTrackdir()));
					}
