 ...
 */
static bool ShipMoveUpDownOnNonReservedLocks(Ship *v)
{
	if (_settings_game.pf.ship_path_reservation) return false;
...
static void ShipController(Ship *v)
{
	...
	if (ShipMoveUpDownOnNonReservedLocks(v)) return;
	...
	if (v->state != TRACK_BIT_WORMHOLE) {
		...
		if (gp.old_tile == gp.new_tile) {
			...
			if (v->current_order.IsType(OT_GOTO_WAYPOINT)) {
				...
				if (HasWaterTrackReservation(gp.new_tile)) LiftReservedWaterPath(gp.new_tile, v->GetVehicleTrackdir());
			...
			if ((tracks & GetReservedWaterTracks(gp.new_tile)) != TRACK_BIT_NONE) {
				...
				LiftCollidingReservedWaterPaths(gp.new_tile, trackdirs);
				...
				if (_settings_game.pf.ship_path_reservation) {
					if (!IsLockTile(gp.old_tile)) {
						...
						* We must lift reservation of all the lock tiles. */
						LiftReservedWaterPath(gp.old_tile, ReverseTrackdir(v->GetVehicleTrackdir()));
					}
