...
enum NWidSizingType {
	...
	NWST_MESSAGE_LENGTH,	///< Size for command errors and message windows.
	NWST_STATUSBAR_MIDDLE,	///< Size for statusbar middle part.
	NWST_STATUSBAR_SIDE,	///< Size for statusbar sides: date and money.
	NWST_VIEWPORT,		///< Size for normal viewports.
	NWST_BIG_NEWS,		///< Size for news with big font.
...
static inline NWidgetPart SetMinimalTextLines(uint8 lines, uint8 spacing, FontSize size = FS_NORMAL, bool scale = 1)
{
	...
	part.u.text_lines.spacing = ScaleGUIPixels(spacing);
