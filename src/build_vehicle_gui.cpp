...
struct BuildVehicleWindow : Window {
		...
		switch (this->vehicle_type) {
			...
			case VEH_TRAIN:
				if (this->listview_mode) {
					this->filter.railtypes = INVALID_RAILTYPES;
				} else {
					this->filter.railtypes = depot->r_types.rail_types;
				}
				break;

			case VEH_ROAD:
				if (this->listview_mode) {
					this->filter.roadtypes = INVALID_ROADTYPES;
				} else {
					this->filter.roadtypes = depot->r_types.road_types;
				}
				break;
	...
	void SelectEngine(EngineID engine)
	{
		...
		if (!this->listview_mode) {
			...
			CommandCost ret = DoCommand(Depot::Get(this->window_number)->xy, this->sel_engine | (cargo << 24), 0, DC_QUERY_COST, GetCmdBuildVeh(this->vehicle_type), nullptr);
			if (ret.Succeeded()) {
	...
	void GenerateBuildAircraftList()
	{
		...
		const Station *st = this->listview_mode ? nullptr : Station::GetByTile(depot->xy);
		...
		switch (widget) {
			...
			case WID_BV_BUILD: {
				...
				if (sel_eng != INVALID_ENGINE) {
					...
					DoCommandP(depot->xy, sel_eng | (cargo << 24), 0, GetCmdBuildVeh(this->vehicle_type), callback);
				}
				break;
			}
