...
struct OrthogonalTileArea {
	...
	void RectExpansion(const OrthogonalTileArea ta);
...
typedef long int CustomBitMap;
assert_compile(CHAR_BIT == 8);

inline CustomBitMap *NewCustomBitMap(uint positions)
{
	return new CustomBitMap[(positions - 1 + CHAR_BIT * sizeof(CustomBitMap)) / (CHAR_BIT * sizeof(CustomBitMap))]();
}

inline void SetBitMapBit(CustomBitMap *bit_map, uint32 position, bool value)
{
	// Check why SB(word, position, 1, value) doesn't work
	if (value) {
		SetBit(bit_map[position / (CHAR_BIT * sizeof(CustomBitMap))], position % (CHAR_BIT * sizeof(CustomBitMap)));
	} else {
		ClrBit(bit_map[position / (CHAR_BIT * sizeof(CustomBitMap))], position % (CHAR_BIT * sizeof(CustomBitMap)));
	}
}

inline bool HasBitMapBit(CustomBitMap *bit_map, uint position)
{
	return HasBit(bit_map[position / (CHAR_BIT * sizeof(CustomBitMap))], position % (CHAR_BIT * sizeof(CustomBitMap)));
}
...
struct BitMapIndex {
	...
	inline BitMapIndex& operator ++() {
		if (++bit_index == sizeof(CustomBitMap) * CHAR_BIT) {
			bit_index = 0;
			word_index++;
		}
		return *this;
 ...
 *             This variable will be allocated in this \c for of this loop.
 * @param ta   The tile area to search over.
 * @param mask The CustomBitMap associated to \c ta. n-th bit is associated to the n-th tile of the tile area loop.
