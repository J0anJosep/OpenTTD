...
struct GraphLegendWindow : Window {
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		...
		DrawCompanyIcon(cid, rtl ? r.right - d.width - ScaleGUIPixels(2) : r.left + ScaleGUIPixels(2), r.top + (r.bottom - r.top - d.height) / 2);

		SetDParam(0, cid);
		SetDParam(1, cid);
		DrawString(r.left + (rtl ? (uint)SWD_FRAMERECT_LEFT : (d.width + ScaleGUIPixels(4))), r.right - (rtl ? (d.width + ScaleGUIPixels(4)) : (uint)SWD_FRAMERECT_RIGHT), r.top + (r.bottom - r.top + 1 - FONT_HEIGHT_NORMAL) / 2, STR_COMPANY_NAME_COMPANY_NUM, HasBit(_legend_excluded_companies, cid) ? TC_BLACK : TC_WHITE);
	}
...
protected:
	...
	void DrawGraph(Rect r) const
	{
		...
		for (int i = 0; i < (num_hori_lines + 1); i++) {
			GfxFillRect(r.left - 3 * WD_GUI_UNIT, y - width_line_offset, r.left - WD_GUI_UNIT, y + width_line_offset, GRAPH_AXIS_LINE_COLOUR);
			GfxFillRect(r.left, y - width_line_offset, r.right, y + width_line_offset, grid_colour);
			y -= y_sep;
		}

		/* Draw the y axis. */
		GfxFillRect(r.left - width_line_offset, r.top, r.left + width_line_offset, r.bottom, GRAPH_AXIS_LINE_COLOUR);
