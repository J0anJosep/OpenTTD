...
struct PaymentRatesGraphWindow : BaseGraphWindow {
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		...
		FOR_ALL_SORTED_STANDARD_CARGOSPECS(cs) {
			...
			byte clk_dif = lowered ? WD_GUI_UNIT : 0;
			...
			GfxFillRect(rect_x + WD_BEVEL, y + WD_BEVEL + clk_dif + text_y_offset, rect_x + font_height - WD_BEVEL, y + clk_dif + text_y_offset + font_height - WD_BEVEL, cs->legend_colour);
			SetDParam(0, cs->name);
