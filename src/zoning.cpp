...
void ResetCatchmentAreaLayer()
{
	...
	_ca_layer = NewCustomBitMap(_map_size);
...
void SetZoning(const Station *st, const TileArea check_area, bool setting_tiles_to)
{
	if (_settings_game.station.precise_catchment) {
		/* With precise catchment, use the catchment footprint. */
		const CustomBitMap *mask = st->GetStationCatchmentFootprint(check_area);
		MASKED_TILE_AREA_LOOP(tile, check_area, mask) {
			SetBitMapBit(_ca_layer, tile, setting_tiles_to);
		}
		delete [] mask;
	} else {
		/* With non-precise catchment, intersect catchment with area. */
		TileArea affected_ta = st->GetStationCatchmentArea();
		affected_ta.IntersectionWith(check_area);
		TILE_AREA_LOOP(tile, affected_ta) {
			SetBitMapBit(_ca_layer, tile, setting_tiles_to);
		}
	}
}
...
void UpdateCatchmentAreaLayer()
{
	...
	for (bool setting_tiles_to = true;; setting_tiles_to = false) {
		...
		for (uint i = _ca_controller.lists[LT_STATIONS].Length(); i--;) {
			if (_ca_controller.lists[LT_STATIONS][i].IsActive() &&
					_ca_controller.lists[LT_STATIONS][i].IsPositive()== setting_tiles_to) {
...
void UpdateCALayer(const TileArea check_area)
{
	...
	for (uint setting_tiles_to = true; ;setting_tiles_to = false) {
		//for each station, update
		for (uint i = involved_stations_state[setting_tiles_to].Length(); i--;) {
			SetZoning(involved_stations_state[setting_tiles_to][i], check_area, setting_tiles_to);
		}
		if (!setting_tiles_to) break;
	}
