...
CommandProc CmdRotateDockTracks;
...
CommandProc CmdBuildGroupsOfVehicleType;
CommandProc CmdAutoGroupsOfVehicleType;
...
static const Command _command_proc_table[] = {
	...
	DEF_CMD(CmdRotateDockTracks,                        CMD_AUTO, CMDT_LANDSCAPE_CONSTRUCTION), // CMD_ROTATE_DOCK_TRACKS
	...
	DEF_CMD(CmdBuildGroupsOfVehicleType,             CMD_OFFLINE, CMDT_ROUTE_MANAGEMENT      ), // CMD_BUILD_GROUPS_OF_VEHICLE_TYPE
	DEF_CMD(CmdAutoGroupsOfVehicleType,              CMD_OFFLINE, CMDT_ROUTE_MANAGEMENT      ), // CMD_AUTOGROUPS_OF_VEHICLE_TYPE
...
bool TouchCommandP(TileIndex tile, uint32 p1, uint32 p2, uint32 cmd, CommandCallback *callback, const char *text, bool my_cmd)
{
	if (_settings_client.gui.touchscreen_mode == TSC_CONFIRM && !_shift_pressed) {
		...
		if (text != NULL) strecpy(_queued_command.text, text, lastof(_queued_command.text));
		_my_cmd = my_cmd;
...
/** Return the tile of the queued command. */
TileIndex GetQueuedCommandTile()
{
	assert(IsValidTile(_queued_command.tile));
	return _queued_command.tile;
}

/** Execute a stored command. Keep it when asking for estimated cost. */
void DoQueuedTouchCommand()
{
	if (!_shift_pressed && DoCommandP(&_queued_command, _my_cmd)) {
