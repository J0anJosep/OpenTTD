...
#ifdef UNFINISHED_AIR_CMD


void ResolveAirTypeGUISprites(AirTypeInfo *rti)
{
...
{
	AirType railtype = Extract<AirType, 0, 4>(p1);
	...
	switch (GetTileType(tile)) {
		case MP_RAILWAY: {
			...
			if (!IsCompatibleRail(GetAirType(tile), railtype)) return_cmd_error(STR_ERROR_IMPOSSIBLE_TRACK_COMBINATION);
			...
			if (GetAirType(tile) != railtype && !HasPowerOnRail(railtype, GetAirType(tile))) {
				if (HasPowerOnRail(GetAirType(tile), railtype)) {
			...
			if (flags & DC_EXEC) {
				...
				Company::Get(GetTileOwner(tile))->infrastructure.rail[GetAirType(tile)] -= pieces;
...
CommandCost CmdRemoveSingleRail(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	switch (GetTileType(tile)) {
		case MP_ROAD: {
			...
			cost.AddCost(RailClearCost(GetAirType(tile)));
			...
			if (flags & DC_EXEC) {
				...
				Company::Get(owner)->infrastructure.rail[GetAirType(tile)] -= LEVELCROSSING_TRACKBIT_FACTOR;
		...
		case MP_RAILWAY: {
			...
			if (flags & DC_EXEC) {
				...
				Company::Get(owner)->infrastructure.rail[GetAirType(tile)] -= pieces;
				/* Add new infrastructure count. */
				present ^= trackbit;
...
static CommandCost CmdRailTrackHelper(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	AirType railtype = Extract<AirType, 0, 4>(p2);
...
/**
 * Convert one air type to the other. You can convert to gravel/asphalt/water/...
 ...
 * @return the cost of this operation or an error
 */
CommandCost CmdConvertAir(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	AirType totype = Extract<AirType, 0, 4>(p2);
	...
	for (; (tile = *iter) != INVALID_TILE; ++(*iter)) {
		...
		AirType type = GetAirType(tile);

		/* Converting to the same type or converting 'hidden' elrail -> rail */
		if (type == totype || (_settings_game.vehicle.disable_elrails && totype == AIRTYPE_BEGIN && type == RAILTYPE_ELECTRIC)) continue;
		...
		switch (tt) {
			...
			case MP_TUNNELBRIDGE: {
				...
				if (!IsCompatibleRail(GetAirType(tile), totype)) {
				...
				if (flags & DC_EXEC) {
					...
					Company *c = Company::Get(GetTileOwner(tile));
					c->infrastructure.rail[GetAirType(tile)] -= num_pieces;
					c->infrastructure.rail[totype] += num_pieces;
					DirtyCompanyInfrastructureWindows(c->index);

					SetAirType(tile, totype);
					SetAirType(endtile, totype);
...
static CommandCost RemoveTrainDepot(TileIndex tile, DoCommandFlag flags)
{
	...
	if (flags & DC_EXEC) {
		...
		Company::Get(owner)->infrastructure.rail[GetAirType(tile)]--;
		DirtyCompanyInfrastructureWindows(owner);
...
static void DrawSingleSignal(TileIndex tile, const AirTypeInfo *rti, Track track, SignalState condition, SignalOffsets image, uint pos)
{
	bool side;
...
static void DrawTrackDetails(const TileInfo *ti, const AirTypeInfo *rti)
{
...
static void DrawTrackBitsOverlay(TileInfo *ti, TrackBits track, const AirTypeInfo *rti)
{
...
{
	const AirTypeInfo *rti = GetAirTypeInfo(GetAirType(ti->tile));
	...
	if (track == 0) {
		...
		if (ti->tileh != SLOPE_FLAT) {
			...
			image = _track_sloped_sprites[ti->tileh - 1] + ati->base_sprites.track_y;
		} else {
			/* track on flat ground */
			(image = ati->base_sprites.track_y, track == TRACK_BIT_Y) ||
			...
			(image = ati->base_sprites.track_ns, track == TRACK_BIT_HORZ) ||
			(image++,                            track == TRACK_BIT_VERT) ||

			(junction = true, false) ||
			(image = ati->base_sprites.ground, (track & TRACK_BIT_3WAY_NE) == 0) ||
		...
		switch (rgt) {
			...
			case RAIL_GROUND_ICE_DESERT: image += ati->snow_offset;  break;
			case RAIL_GROUND_WATER: {
				/* three-corner-raised slope */
				DrawShoreTile(ti->tileh);
	...
	if (_game_mode != GM_MENU && _settings_client.gui.show_track_reservation && WaterTrackMayExist(ti->tile)) DrawWaterTrackReservation(ti->tile);

	/* Draw track pieces individually for junction tiles */
	if (junction) {
		if (track & TRACK_BIT_X)     DrawGroundSprite(ati->base_sprites.single_x, PAL_NONE);
		if (track & TRACK_BIT_Y)     DrawGroundSprite(ati->base_sprites.single_y, PAL_NONE);
		if (track & TRACK_BIT_UPPER) DrawGroundSprite(ati->base_sprites.single_n, PAL_NONE);
		if (track & TRACK_BIT_LOWER) DrawGroundSprite(ati->base_sprites.single_s, PAL_NONE);
		if (track & TRACK_BIT_LEFT)  DrawGroundSprite(ati->base_sprites.single_w, PAL_NONE);
		if (track & TRACK_BIT_RIGHT) DrawGroundSprite(ati->base_sprites.single_e, PAL_NONE);
	}
	...
	if (_game_mode != GM_MENU && _settings_client.gui.show_track_reservation) {
		...
		if (pbs & TRACK_BIT_X) {
			if (ti->tileh == SLOPE_FLAT || ti->tileh == SLOPE_ELEVATED) {
				DrawGroundSprite(ati->base_sprites.single_x, PALETTE_CRASH);
			} else {
				DrawGroundSprite(_track_sloped_sprites[ti->tileh - 1] + ati->base_sprites.single_sloped - 20, PALETTE_CRASH);
			}
		}
		if (pbs & TRACK_BIT_Y) {
			if (ti->tileh == SLOPE_FLAT || ti->tileh == SLOPE_ELEVATED) {
				DrawGroundSprite(ati->base_sprites.single_y, PALETTE_CRASH);
			} else {
				DrawGroundSprite(_track_sloped_sprites[ti->tileh - 1] + ati->base_sprites.single_sloped - 20, PALETTE_CRASH);
			}
		}
		if (pbs & TRACK_BIT_UPPER) DrawGroundSprite(ati->base_sprites.single_n, PALETTE_CRASH, NULL, 0, ti->tileh & SLOPE_N ? -(int)TILE_HEIGHT : 0);
		if (pbs & TRACK_BIT_LOWER) DrawGroundSprite(ati->base_sprites.single_s, PALETTE_CRASH, NULL, 0, ti->tileh & SLOPE_S ? -(int)TILE_HEIGHT : 0);
		if (pbs & TRACK_BIT_LEFT)  DrawGroundSprite(ati->base_sprites.single_w, PALETTE_CRASH, NULL, 0, ti->tileh & SLOPE_W ? -(int)TILE_HEIGHT : 0);
		if (pbs & TRACK_BIT_RIGHT) DrawGroundSprite(ati->base_sprites.single_e, PALETTE_CRASH, NULL, 0, ti->tileh & SLOPE_E ? -(int)TILE_HEIGHT : 0);
	}
	...
	if (IsValidCorner(halftile_corner)) {
		...
		/* Draw higher halftile-overlay: Use the sloped sprites with three corners raised. They probably best fit the lightning. */
		Slope fake_slope = SlopeWithThreeCornersRaised(OppositeCorner(halftile_corner));
		image = _track_sloped_sprites[fake_slope - 1] + ati->base_sprites.track_y;
		pal = PAL_NONE;
		switch (rgt) {
			case RAIL_GROUND_BARREN:     pal = PALETTE_TO_BARE_LAND; break;
			case RAIL_GROUND_ICE_DESERT:
			case RAIL_GROUND_HALF_SNOW:  image += ati->snow_offset;  break; // higher part has snow in this case too
			default: break;
		}
		...
		if (_game_mode != GM_MENU && _settings_client.gui.show_track_reservation && HasReservedTracks(ti->tile, CornerToTrackBits(halftile_corner))) {
			...
			DrawGroundSprite(_corner_to_track_sprite[halftile_corner] + ati->base_sprites.single_n, PALETTE_CRASH, NULL, 0, -(int)TILE_HEIGHT);
		}
	}
}

static void DrawSignals(TileIndex tile, TrackBits rails, const AirTypeInfo *rti)
{
...
static void DrawTile_Track(TileInfo *ti)
{
	...
	if (IsPlainRail(ti->tile)) {
		...
		if (HasCatenaryDrawn(GetAirType(ti->tile))) DrawCatenary(ti);
		...
		if (ati->UsesOverlay()) {
			...
			if (image != SPR_FLAT_GRASS_TILE) image += ati->GetRailtypeSpriteOffset();
		}
		...
		if (IsSnowRailGround(ti->tile) && _settings_game.game_creation.landscape == LT_TROPIC) {
			if (image != SPR_FLAT_GRASS_TILE) {
				image += ati->snow_offset; // tile with tracks
			} else {
		...
		if (ati->UsesOverlay()) {
			...
			if (_game_mode != GM_MENU && _settings_client.gui.show_track_reservation && HasDepotReservation(ti->tile)) {
				switch (GetRailDepotDirection(ti->tile)) {
					case DIAGDIR_NE: if (!IsInvisibilitySet(TO_BUILDINGS)) break; // else FALL THROUGH
					case DIAGDIR_SW: DrawGroundSprite(ati->base_sprites.single_x, PALETTE_CRASH); break;
					case DIAGDIR_NW: if (!IsInvisibilitySet(TO_BUILDINGS)) break; // else FALL THROUGH
					case DIAGDIR_SE: DrawGroundSprite(ati->base_sprites.single_y, PALETTE_CRASH); break;
					default: break;
				}
			}
		}
		int depot_sprite = GetCustomRailSprite(rti, ti->tile, RTSG_DEPOT);
		relocation = depot_sprite != 0 ? depot_sprite - SPR_RAIL_DEPOT_SE_1 : ati->GetRailtypeSpriteOffset();
...
void DrawTrainDepotSprite(int x, int y, int dir, AirType railtype)
{
	const DrawTileSprites *dts = &_depot_gfx_table[dir];
	const AirTypeInfo *rti = GetAirTypeInfo(railtype);
	SpriteID image = ati->UsesOverlay() ? SPR_FLAT_GRASS_TILE : dts->ground.sprite;
	uint32 offset = ati->GetRailtypeSpriteOffset();
...
{
	const AirTypeInfo *rti = GetAirTypeInfo(GetAirType(tile));
	td->rail_speed = ati->max_speed;
	td->owner[0] = GetTileOwner(tile);
	SetDParamX(td->dparam, 0, ati->strings.name);
	switch (GetRailTileType(tile)) {
		case RAIL_TILE_NORMAL:
...
/**
 * Test-procedure for HasVehicleOnPos to check for an aircraft.
 */
static Vehicle *EnsureNoAircraftProc(Vehicle *v, void *data)
{
	return v->type == VEH_AIRCRAFT ? v : NULL;
}
...
extern const TileTypeProcs _tile_type_air_procs = {
