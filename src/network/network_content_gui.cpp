...
/* virtual */ void BaseNetworkContentDownloadStatusWindow::DrawWidget(const Rect &r, int widget) const
{
	...
	DrawFrameRect(r.left + ScaleGUIPixels(20), r.top + ScaleGUIPixels(4), r.left + ScaleGUIPixels(20) + (int)(this->width - ScaleGUIPixels(40LL)) * this->downloaded_bytes / this->total_bytes, r.top + ScaleGUIPixels(14), COLOUR_MAUVE, FR_NONE);

	int y = r.top + ScaleGUIPixels(20);
	SetDParam(0, this->downloaded_bytes);
	SetDParam(1, this->total_bytes);
	SetDParam(2, this->downloaded_bytes * 100LL / this->total_bytes);
	DrawString(r.left + ScaleGUIPixels(2), r.right - ScaleGUIPixels(2), y, STR_CONTENT_DOWNLOAD_PROGRESS_SIZE, TC_FROMSTRING, SA_HOR_CENTER);
	...
	y += FONT_HEIGHT_NORMAL + ScaleGUIPixels(5);
	DrawStringMultiLine(r.left + ScaleGUIPixels(2), r.right - ScaleGUIPixels(2), y, y + FONT_HEIGHT_NORMAL * 2, str, TC_FROMSTRING, SA_CENTER);
}
...
public:
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		switch (widget) {
			...
			case WID_NCL_CHECKBOX:
				size->width = this->checkbox_size.width + ScaleGUIPixels(WD_MATRIX_RIGHT + WD_MATRIX_LEFT);
				break;
			...
			case WID_NCL_TYPE: {
				...
				size->width = d.width + ScaleGUIPixels(WD_MATRIX_RIGHT + WD_MATRIX_LEFT);
				break;
			}

			case WID_NCL_MATRIX:
				resize->height = GetMinSizing(NWST_STEP, max(this->checkbox_size.height, (uint)FONT_HEIGHT_NORMAL) + ScaleGUIPixels(WD_MATRIX_TOP + WD_MATRIX_BOTTOM));
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		switch (widget) {
			case WID_NCL_FILTER_CAPT:
				DrawString(r.left, r.right, CenterBounds(r.top, r.bottom), STR_CONTENT_FILTER_TITLE, TC_FROMSTRING, SA_RIGHT);
				break;
	...
	void DrawMatrix(const Rect &r) const
	{
		...
		for (ConstContentIterator iter = this->content.Get(this->vscroll->GetPosition()); iter != this->content.End() && cnt < this->vscroll->GetCapacity(); iter++, cnt++) {
			...
			if (ci == this->selected) GfxFillRect(r.left + 1, y + SWD_FRAMERECT_TOP, r.right - 1, y + this->resize.step_height - SWD_FRAMERECT_BOTTOM, PC_GREY);
			...
			DrawString(nwi_name->pos_x + SWD_FRAMERECT_LEFT, nwi_name->pos_x + nwi_name->current_x - SWD_FRAMERECT_RIGHT, Center(y,this->resize.step_height), ci->name, TC_BLACK);
	...
	void DrawDetails(const Rect &r) const
	{
		...
		GfxFillRect(r.left + WD_GUI_UNIT, r.top + WD_GUI_UNIT, r.right - WD_GUI_UNIT, r.top + DETAIL_TITLE_HEIGHT, PC_DARK_BLUE);
		DrawString(r.left + SWD_INSET_LEFT, r.right - SWD_INSET_RIGHT, r.top + FONT_HEIGHT_NORMAL + SWD_INSET_TOP, STR_CONTENT_DETAIL_TITLE, TC_FROMSTRING, SA_HOR_CENTER);

		/* Draw the total download size */
		SetDParam(0, this->filesize_sum);
		DrawString(r.left + DETAIL_LEFT, r.right - DETAIL_RIGHT, r.bottom - FONT_HEIGHT_NORMAL - SWD_PAR_VSEP_NORMAL, STR_CONTENT_TOTAL_DOWNLOAD_SIZE);

		if (this->selected == NULL) return;

		/* And fill the rest of the details when there's information to place there */
		DrawStringMultiLine(r.left + SWD_INSET_LEFT, r.right - SWD_INSET_RIGHT, r.top + DETAIL_TITLE_HEIGHT / 2, r.top + DETAIL_TITLE_HEIGHT, STR_CONTENT_DETAIL_SUBTITLE_UNSELECTED + this->selected->state, TC_FROMSTRING, SA_CENTER);

		/* Also show the total download size, so keep some space from the bottom */
		const uint max_y = r.bottom - FONT_HEIGHT_NORMAL - SWD_PAR_VSEP_WIDE;
	...
	NWidget(WWT_PANEL, COLOUR_LIGHT_BLUE, WID_NCL_BACKGROUND),
		...
		NWidget(NWID_HORIZONTAL, NC_EQUALSIZE), SetPIP(8, 8, 8),
			...
			NWidget(NWID_VERTICAL), SetPIP(0, 4, 0),
				...
				NWidget(NWID_HORIZONTAL, NC_EQUALSIZE), SetPIP(0, 8, 0),
					NWidget(NWID_SELECTION, INVALID_COLOUR, WID_NCL_SEL_ALL_UPDATE), SetResize(1, 0), SetFill(1, 0),
						NWidget(WWT_PUSHTXTBTN, COLOUR_LIGHT_BLUE, WID_NCL_SELECT_UPDATE), SetResize(1, 0), SetFill(1, 0),
												SetDataTip(STR_CONTENT_SELECT_UPDATES_CAPTION, STR_CONTENT_SELECT_UPDATES_CAPTION_TOOLTIP),
						NWidget(WWT_PUSHTXTBTN, COLOUR_LIGHT_BLUE, WID_NCL_SELECT_ALL), SetResize(1, 0), SetFill(1, 0),
												SetDataTip(STR_CONTENT_SELECT_ALL_CAPTION, STR_CONTENT_SELECT_ALL_CAPTION_TOOLTIP),
					EndContainer(),
					NWidget(WWT_PUSHTXTBTN, COLOUR_LIGHT_BLUE, WID_NCL_UNSELECT), SetResize(1, 0), SetFill(1, 0),
			...
			NWidget(NWID_VERTICAL), SetPIP(0, 4, 0),
				...
				NWidget(NWID_HORIZONTAL, NC_EQUALSIZE), SetPIP(0, 8, 0),
					NWidget(WWT_PUSHTXTBTN, COLOUR_LIGHT_BLUE, WID_NCL_TEXTFILE + TFT_README), SetFill(1, 0), SetResize(1, 0), SetDataTip(STR_TEXTFILE_VIEW_README, STR_NULL),
					NWidget(WWT_PUSHTXTBTN, COLOUR_LIGHT_BLUE, WID_NCL_TEXTFILE + TFT_CHANGELOG), SetFill(1, 0), SetResize(1, 0), SetDataTip(STR_TEXTFILE_VIEW_CHANGELOG, STR_NULL),
				EndContainer(),
				NWidget(NWID_HORIZONTAL, NC_EQUALSIZE), SetPIP(0, 8, 0),
					NWidget(WWT_PUSHTXTBTN, COLOUR_LIGHT_BLUE, WID_NCL_OPEN_URL), SetResize(1, 0), SetFill(1, 0), SetDataTip(STR_CONTENT_OPEN_URL, STR_CONTENT_OPEN_URL_TOOLTIP),
					NWidget(WWT_PUSHTXTBTN, COLOUR_LIGHT_BLUE, WID_NCL_TEXTFILE + TFT_LICENSE), SetFill(1, 0), SetResize(1, 0), SetDataTip(STR_TEXTFILE_VIEW_LICENCE, STR_NULL),
				EndContainer(),
			EndContainer(),
		EndContainer(),
		NWidget(NWID_SPACER), SetMinimalSize(0, 7), SetResize(1, 0),
		/* Bottom. */
		NWidget(NWID_HORIZONTAL, NC_EQUALSIZE), SetPIP(8, 8, 8),
			NWidget(WWT_PUSHTXTBTN, COLOUR_LIGHT_BLUE, WID_NCL_SEARCH_EXTERNAL), SetResize(1, 0), SetFill(1, 0),
										SetDataTip(STR_CONTENT_SEARCH_EXTERNAL, STR_CONTENT_SEARCH_EXTERNAL_TOOLTIP),
			NWidget(NWID_HORIZONTAL, NC_EQUALSIZE), SetPIP(0, 8, 0),
				NWidget(WWT_PUSHTXTBTN, COLOUR_LIGHT_BLUE, WID_NCL_CANCEL), SetResize(1, 0), SetFill(1, 0),
											SetDataTip(STR_BUTTON_CANCEL, STR_NULL),
				NWidget(WWT_PUSHTXTBTN, COLOUR_LIGHT_BLUE, WID_NCL_DOWNLOAD), SetResize(1, 0), SetFill(1, 0),
