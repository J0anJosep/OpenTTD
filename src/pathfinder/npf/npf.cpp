	...
	switch (GetTileType(current->tile)) {
		...
		case MP_WATER:
			if (IsShipDepot(current->tile)) {
				if (IsExtendedDepot(current->tile) && IsDepotFullWithStoppedVehicles(current->tile)) cost += NPF_INFINITE_PENALTY;
			}
			break;
	}
...
static int32 NPFRailPathCost(AyStar *as, AyStarNode *current, OpenListNode *parent)
{
	...
	if (IsStandardRailDepotTile(tile) && as->EndNodeCheck(as, &new_node) != AYSTAR_FOUND_END_NODE) {
