...
Trackdir YapfAircraftFindPath(const Aircraft *v, PBSTileInfo &best_dest, bool &path_found, AircraftState dest_state, AircraftPathChoice &path_cache)
{
	...
	typedef Trackdir (*PfnChooseAircraftPath)(const Aircraft *, PBSTileInfo *, bool &, AircraftState, AircraftPathChoice &);
	PfnChooseAircraftPath pfnChooseAircraftPath = &CYapfAircraft::ChooseAircraftPath; // Trackdir, allow 90-deg, check for reservations

	return pfnChooseAircraftPath(v, &best_dest, path_found, dest_state, path_cache);
}
