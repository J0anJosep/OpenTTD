...
static void DrawYearColumn(const Rect &r, int year, const Money (*tbl)[EXPENSES_END])
{
	...
	for (uint i = 0; i < _expenses_list_types[type].length; i++) {
		...
		if (et == INVALID_EXPENSES) {
			Money cost = subtotal;
			subtotal = 0;
			GfxFillRect(r.left, y, r.right, y + WD_BEVEL - 1, PC_BLACK);
			y += EXP_LINESPACE;
...
static const NWidgetPart _nested_company_finances_widgets[] = {
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_TEXTBTN, COLOUR_GREY, WID_CF_TOGGLE_SIZE), SetDataTip(STR_ICON_WINDOW, STR_TOOLTIP_TOGGLE_LARGE_SMALL_WINDOW),
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		switch (widget) {
			...
			case WID_CF_LOAN_LINE:
				GfxFillRect(r.left, r.top, r.right, r.top + WD_BEVEL - 1, PC_BLACK);
				break;
		}
	}
...
public:
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		switch (widget) {
			case WID_SCL_SPACER_DROPDOWN: {
				...
				size->width = max(size->width, ScaleGUIPixels(5 + WD_FRAMERECT_RIGHT) + this->square.width + d.width);
				break;
			}
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		...
		int text_left  = (rtl ? (uint)SWD_FRAMERECT_LEFT : (this->square.width + ScaleGUIPixels(5)));
		int text_right = (rtl ? (this->square.width + ScaleGUIPixels(5)) : (uint)SWD_FRAMERECT_RIGHT);
		...
		auto draw_livery = [&](StringID str, const Livery &liv, bool sel, bool def, int indent) {
			/* Livery Label. */
			DrawString(sch_left + SWD_FRAMERECT_LEFT + (rtl ? 0 : ScaleGUIPixels(indent)), sch_right - SWD_FRAMERECT_RIGHT - (rtl ? ScaleGUIPixels(indent) : 0), y + text_offs, str, sel ? TC_WHITE : TC_BLACK);

			/* Text below the first dropdown. */
			DrawSprite(SPR_SQUARE, GENERAL_SPRITE_COLOUR(liv.colour1), (rtl ? pri_right - (this->square.width + 5) + SWD_FRAMERECT_RIGHT : pri_left) + SWD_FRAMERECT_LEFT, y + square_offs);
			...
			if (sec_right > sec_left) { // Second dropdown has non-zero size.
				DrawSprite(SPR_SQUARE, GENERAL_SPRITE_COLOUR(liv.colour2), (rtl ? sec_right - (this->square.width + 5) + SWD_FRAMERECT_RIGHT : sec_left) + SWD_FRAMERECT_LEFT, y + square_offs);
...
static const NWidgetPart _nested_select_company_manager_face_widgets[] = {
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_TEXTBTN, COLOUR_GREY, WID_SCMF_TOGGLE_LARGE_SMALL), SetDataTip(STR_ICON_WINDOW, STR_FACE_ADVANCED_TOOLTIP),
	EndContainer(),
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		...
		switch (widget) {
			...
			case WID_CI_TOTAL:
				if (_settings_game.economy.infrastructure_maintenance) {
					int left = _current_text_dir == TD_RTL ? r.right - this->total_width : r.left;
					GfxFillRect(left, y, left + this->total_width, y + WD_BEVEL - 1, PC_WHITE);
					y += EXP_LINESPACE;
...
struct CompanyWindow : Window
{
	...
	{
		if (TouchCommandP(tile, OBJECT_HQ, 0, CMD_BUILD_OBJECT | CMD_MSG(STR_ERROR_CAN_T_BUILD_COMPANY_HEADQUARTERS)) && !_shift_pressed) {
			ResetObjectToPlace();
			this->RaiseButtons();
		}
	}
