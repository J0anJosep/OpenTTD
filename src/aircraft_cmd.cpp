...
void Aircraft::FreeReservation()
{
	if (this->cur_state == AM_HANGAR) {
		if (IsBigHangar(this->tile)) {
			...
			if (_show_airport_tracks) MarkTileDirtyByTile(this->tile);
		}
	} else {
		// free path
	}
 ...
 */
static bool CheckPartialDestination(const Aircraft *v)
{
	...
	if (v->cur_state == AM_HELICOPTER_TAKEOFF) return false;
 ...
 * @return true if it is at its desired state, false if it needs to move towards another tile.
 */
static bool CheckDestination(const Aircraft *v)
{
...
void DumpAircraftState(const Aircraft *v)
{
	...
	DEBUG(misc, 0, "Last byte %d %d", v->x_pos & 0xF, v->y_pos & 0xF);
	DEBUG(misc, 0, "CheckDest %d, checkpartial %d", CheckDestination(v), CheckPartialDestination(v));
}
...
bool TryReservePath(Aircraft *v)
{
	...
	if (v->cur_state == AM_HANGAR) {
		...
		assert(GetReservedAirportTracks(v->tile) != TRACK_BIT_NONE);
	}
	...
	if (v->cur_state == AM_HANGAR) {
		...
		if (IsBigHangar(v->tile)) {
			...
			v->trackdir = v->Next()->next_trackdir = DiagDirToDiagTrackdir(GetHangarDirection(v->tile));
			v->SetHidden(false);
...
TileIndex GetClosestLandingTile(Aircraft *v)
{
	...
	uint32 free_best_dist = UINT32_MAX;
	...
	if (v->IsHelicopter()) {
		for (uint i = st->airport.helipads.Length(); i--;) {
			...
			if (!HasAirportTrackReserved(st->airport.helipads[i]) &&
					DistanceSquare(st->airport.helipads[i], v->tile) < free_best_dist) {
				free_landing_tile = st->airport.helipads[i];
				free_best_dist = DistanceSquare(st->airport.helipads[i], v->tile);
			}
		}
		...
		for (uint i = st->airport.terminals.Length(); i--;) {
			...
			if (!HasAirportTrackReserved(st->airport.terminals[i]) &&
					DistanceSquare(st->airport.terminals[i], v->tile) < free_best_dist) {
				free_landing_tile = st->airport.terminals[i];
				free_best_dist = DistanceSquare(st->airport.terminals[i], v->tile);
			}
		}
	...
	for (uint i = st->airport.runways.Length(); i--;) {
		...
		if (!HasAirportTrackReserved(st->airport.runways[i]) &&
				DistanceSquare(st->airport.runways[i], v->tile) < free_best_dist) {
			free_landing_tile = st->airport.runways[i];
			free_best_dist = DistanceSquare(st->airport.runways[i], v->tile);
		}
	}
...
Trackdir GetFreeTrackdir(TileIndex tile, Trackdir preferred_trackdir)
{
	...
	if ((TrackToTrackBits(preferred_track) & tracks) != 0) {
		if (IsAirportPositionFree(tile, preferred_track)) return preferred_trackdir;
	}
	...
	if (tracks != TRACK_BIT_NONE) {
		...
		if (IsAirportPositionFree(tile, preferred_track)) return TrackToTrackdir(preferred_track);
		assert(tracks == TRACK_BIT_NONE);
	}
	...
	if (v->targetairport != GetStationIndex(v->tile)) {
		...
		if (v->IsHelicopter()) {
			if (IsTerminal(v->tile) && (IsHelipad(v->tile) ||
				...
				v->target_state = AM_HELICOPTER_TAKEOFF;
			}
		} else if (!IsRunwayStart(v->tile)) {
			//if (v->cur_state != AM_HANGAR) v->cur_state = AM_IDLE;
			v->next_state = AM_TAKEOFF;
			v->next_tile = INVALID_TILE;
			v->dest_tile = INVALID_TILE;
		}
	} else {
		switch (v->target_state) {
			case AM_TERMINAL:
				assert(IsTileType(v->tile, MP_STATION) && IsAirportTile(v->tile));
 ...
 * @param v aircraft.
 */
bool AircraftArrivesAtPartialDestination(Aircraft *v)
{
	switch (v->next_state) {
		default:
			break;
	...
	switch (v->cur_state) {
		case AM_FLYING:
			if (v->next_state == AM_LANDING) {
				...
				if (IsAirportClosed(v->tile) || !CanRunwayBeReserved(v->tile)) {
					//v->next_tile = v->tile;
					//DumpAircraftState(v);
					return true;
				}
		...
		if (GetStationIndex(v->tile) != v->targetairport) {
			if (v->IsAircraftFlying()) {
				...
				if (v->IsHelicopter()) {
					if (IsTerminal(v->tile)) {
						v->cur_state = AM_HELICOPTER_TAKEOFF;
						v->next_state = AM_HELICOPTER_TAKEOFF;
					} else {
						v->next_state = AM_HELICOPTER_TAKEOFF;
					}
				} else {
	...
	switch (v->cur_state) {
		...
		case AM_MOVING:
			if (v->next_state == AM_HANGAR) {
				...
				v->next_trackdir = DiagDirToDiagTrackdir(GetRunwayExtremeDirection(v->tile));
				if (DiagDirToDir(GetRunwayExtremeDirection(v->tile)) != v->direction) {
					v->desired_trackdir = DiagDirToDiagTrackdir(GetRunwayExtremeDirection(v->tile));
		...
		case AM_FLYING:
			...
			if (v->next_state == AM_LANDING) {
				assert(IsRunwayStart(v->tile));
				if (IsAirportClosed(v->tile)) NOT_REACHED();
				if (!CanRunwayBeReserved(v->tile)) NOT_REACHED();
				SetPlatformReservation(v->tile, INVALID_DIAGDIR, true);
				v->next_trackdir = DiagDirToDiagTrackdir(GetRunwayExtremeDirection(v->tile));
				v->Next()->next_trackdir = v->next_trackdir;
				...
				Trackdir trackdir = DiagDirToDiagTrackdir(DirToDiagDir(v->direction));
				assert(IsDiagonalTrackdir(trackdir));
...
static void AircraftController(Aircraft *v, bool mode)
{
	...
	if (CheckPartialDestination(v)) {
		if (AircraftArrivesAtPartialDestination(v)) return;
