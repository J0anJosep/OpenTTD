...
static CommandCost ReplaceChain(Vehicle **chain, DoCommandFlag flags, bool wagon_removal, bool *nothing_to_do)
{
	...
	if (old_head->type == VEH_TRAIN) {
		...
		if ((flags & DC_EXEC) == 0) {
			for (int i = num_units - 1; i >= 0; i--) {
				if (new_vehs[i] != nullptr) {
					Command<CMD_SELL_VEHICLE>::Do(DC_EXEC | DC_AUTOREPLACE, new_vehs[i]->index, false, false, INVALID_CLIENT_ID);
					new_vehs[i] = nullptr;
				}
			}
		}
...
CommandCost CmdAutoreplaceVehicle(DoCommandFlag flags, VehicleID veh_id)
{
	...
	if (IsExtendedDepotTile(v->tile)) SetExtendedDepotReservation(v, false);
	...
	if (IsExtendedDepotTile(v->tile)) SetExtendedDepotReservation(v, true);
